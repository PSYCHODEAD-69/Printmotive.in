...
function showOrderModal(product, price, desc) {
  ...
  // WA button — delivery popup pehle
  const btnWA = modal.querySelector("#pmBtnWA");
  if (btnWA) {
    btnWA.addEventListener("click", function(e) {
      e.preventDefault();
      showDeliveryPopup("wa", buildWALink(product, price, desc), null, product, price);
      closeModal();
    });
  }

  // Email button — delivery popup pehle
  const btnEmail = modal.querySelector(".pm-btn-email");
  if (btnEmail) {
    btnEmail.addEventListener("click", function(e) {
      e.preventDefault();
      showDeliveryPopup("email", null, buildEmailLink(product, price, desc), product, price);
      closeModal();
    });
  }

  // Instagram — seedha redirect
...
function addToCart(el) {
  ...
  if (existing) {
    ...
    cart.push({ product, price, priceNum, desc, qty: 1 });
  }
  saveCart();
  updateCartBadge();
  addRipple(el);
  ...
  if (cart.length === 0) {
    itemsEl.innerHTML = `
      <div class="cart-empty">
        <svg viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="1.5" width="52" height="52"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
  ...
  itemsEl.innerHTML = cart.map((item, idx) => `
    <div class="cart-item">
      <div class="ci-info">
        ...
        <div class="ci-price">${item.price} each &nbsp;·&nbsp; <span class="ci-subtotal">Rs.${item.priceNum * item.qty}</span></div>
      </div>
      <div class="ci-qty">
...
function buildCartMsg() {
  ...
  return [
    ...
    ``,
    ...cart.map(i => `• ${i.product} x${i.qty}  —  ${i.price} each  =  Rs.${i.priceNum * i.qty}`),
    ``,
...
function cartOrderWA() {
  ...
  closeCart();
  showDeliveryPopup("wa", `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(cartLines)}`, null, null, null, true);
}
...
/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════ */
function showDeliveryPopup(type, waLink, emailLink, product, price, isCart) {
  ...
  popup.innerHTML = `
    ...
    <div class="pm-dp-box">
      ...
      <p class="pm-dp-sub">Please fill in your details to complete the order</p>
      ...
      <textarea           id="dpAddress" class="pm-dp-input pm-dp-textarea" rows="2" placeholder="Delivery Address *" maxlength="200"></textarea>
  ...
  popup.querySelector("#dpConfirm").addEventListener("click", function() {
    ...
    // Delivery details ko existing message mein append karo
    function appendDetails(link) {
      const details = `\n---\nOrder by: ${name}\nPhone: ${phone}\nDelivery Address: ${address}`;
      if (link.startsWith("https://wa.me/")) {
        const urlObj  = new URL(link);
        const oldText = decodeURIComponent(urlObj.searchParams.get("text") || "");
        urlObj.searchParams.set("text", oldText + details);
        return urlObj.toString();
      } else if (link.startsWith("mailto:")) {
        const urlObj  = new URL(link);
        const oldBody = decodeURIComponent(urlObj.searchParams.get("body") || "");
        urlObj.searchParams.set("body", oldBody + details);
        return urlObj.toString();
      }
      return link;
    }
    ...
    if (type === "wa" && waLink) {
      window.open(appendDetails(waLink), "_blank");
    } else if (type === "email" && emailLink) {
      window.open(appendDetails(emailLink));
    }
